...
class MCFF(nn.Module):
    def __init__(self, c1, c2, r=2):
        ...
        self.fc = nn.Sequential(
            # nn.Conv2d(c1*2, c1//r, 1),
            # nn.ReLU(inplace=True),
            # nn.Conv2d(c1//r, c1*2, 1)
            nn.Linear(c1*2, c1//r),
            # nn.ReLU(inplace=True),
            nn.Linear(c1//r, c1*2)
            # nn.Linear(c1*2, c1*2)
...
class Inter_MA(nn.Module):
    ...
    def forward(self, x):
        ...
        f_d = x[0] - x[1]
        v_d = self.gap(f_d)
        v_d = self.fc(v_d.view(bs, c))
        # feat = self.fc(feat)
        
        f_fusion = x[0]*v_d.view(bs, c, 1, 1) + x[1]
        return f_fusion
